...
def register(subparsers: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    p = subparsers.add_parser("build-context", help="Retrieve + rerank + build context")
...
def handle(
    ...
    result = orchestrator.run_through_context(
        query=args.query,
    ...
    if result.builder_result:
        renderer.render_builder_result(result.builder_result, result.trace_id)
